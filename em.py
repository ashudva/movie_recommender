...
def run(X: np.ndarray, mixture: GaussianMixture,
    ...
    ll = None
    ll_history = []
    # pbr = tqdm(desc='EM run:', total=27, leave=False)
    i = 0
    while (prev_ll is None or ll - prev_ll > 1e-6 * np.abs(ll)):
        pbr = tqdm(
            desc=f'EM Run[{i}] E-Step and M-Step:', total=2, leave=False)
        prev_ll = ll
        ...
        pbr.close()
        i += 1
    print(f"Iterations till EM convergence: {i}")
...
def fill_matrix(X: np.ndarray, mixture: GaussianMixture) -> np.ndarray:
    ...
    X_pred = X.copy()
    K, _ = mixture.mu.shape
    pbr = tqdm(range(n), desc='Matrix Completetion', total=n, leave=True)
    for i in pbr:
        mask = X[i, :] != 0
        mask0 = X[i, :] == 0
        post = np.zeros(K)
